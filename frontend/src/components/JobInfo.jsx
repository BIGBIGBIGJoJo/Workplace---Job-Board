...
const JobInfo = ({ job }) => {
  ...
  const companyProfile =
    job.company?.profile || "This employer is hiring through Workplace Job Board.";
  const requirements =
    Array.isArray(job.requirement) && job.requirement.length > 0
      ? job.requirement
        ...
        <div className="m-9">
          <div className="text-center font-mono mb-12">
            ...
            <div className="mt-8 text-sm text-black bg-blue-100 px-5 py-2 rounded-full inline-block shadow-sm">
              <p>Posting Date: <span className="font-semibold text-gray-600 italic">{job.postDate || "Recently posted"}</span></p>
            </div>
          </div>
          ...
          <footer className="font-mono text-center text-gray-700 bg-blue-50 p-8 rounded-t-xl mt-10 shadow-sm">
            ...
            <p className="mb-6 text-xl font-extralight">
              Nature of Business: <span className="font-bold text-blue-700">{job.nature || "General"}</span>
            </p>
